...
public interface StatisticsJpaRepository {
    ...
    @Repository
    interface DailyStatisticsJpaRepository extends JpaRepository<DailyStatisticsJpaEntity, Long> {
        ...
        @Query(
                "SELECT d FROM DailyStatisticsJpaEntity d WHERE d.recordKey = :recordKey "
                        + "AND YEAR(d.statisticsDate) = :year AND MONTH(d.statisticsDate) = :month "
    ...
    @Repository
    interface MonthlyStatisticsJpaRepository
            extends JpaRepository<MonthlyStatisticsJpaEntity, Long> {
